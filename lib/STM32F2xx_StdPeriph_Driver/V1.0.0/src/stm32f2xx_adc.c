...
void ADC_Init(ADC_TypeDef* ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
  ...
  /*
  assert(IS_ADC_ALL_PERIPH(ADCx));
  assert(IS_ADC_RESOLUTION(ADC_InitStruct->ADC_Resolution)); 
  ...
  assert(IS_ADC_EXT_TRIG_EDGE(ADC_InitStruct->ADC_ExternalTrigConvEdge)); 
  assert(IS_ADC_EXT_TRIG(ADC_InitStruct->ADC_ExternalTrigConv));    
  assert(IS_ADC_DATA_ALIGN(ADC_InitStruct->ADC_DataAlign)); 
  assert(IS_ADC_REGULAR_LENGTH(ADC_InitStruct->ADC_NbrOfConversion));
  */
...
void ADC_CommonInit(ADC_CommonInitTypeDef* ADC_CommonInitStruct)
{
  ...
  /* Check the parameters */
  /*
  assert(IS_ADC_MODE(ADC_CommonInitStruct->ADC_Mode));
  assert(IS_ADC_PRESCALER(ADC_CommonInitStruct->ADC_Prescaler));
  assert(IS_ADC_DMA_ACCESS_MODE(ADC_CommonInitStruct->ADC_DMAAccessMode));
  assert(IS_ADC_SAMPLING_DELAY(ADC_CommonInitStruct->ADC_TwoSamplingDelay));
  */
...
void ADC_ExternalTrigInjectedConvEdgeConfig(ADC_TypeDef* ADCx, uint32_t ADC_ExternalTrigInjecConvEdge)
{
  ...
  /* Check the parameters */
  //assert(IS_ADC_ALL_PERIPH(ADCx));
  //assert(IS_ADC_EXT_INJEC_TRIG_EDGE(ADC_ExternalTrigInjecConvEdge));
  /* Get the old register value */
  tmpreg = ADCx->CR2;
