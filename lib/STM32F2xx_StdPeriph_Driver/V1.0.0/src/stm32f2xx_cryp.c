...
void CRYP_Init(CRYP_InitTypeDef* CRYP_InitStruct)
{
  ...
  //assert(IS_CRYP_ALGOMODE(CRYP_InitStruct->CRYP_AlgoMode));
  //assert(IS_CRYP_DATATYPE(CRYP_InitStruct->CRYP_DataType));
  //assert(IS_CRYP_ALGODIR(CRYP_InitStruct->CRYP_AlgoDir));
  ...
  {
    //assert(IS_CRYP_KEYSIZE(CRYP_InitStruct->CRYP_KeySize));
...
{
  /* Check the parameters */
  //assert(IS_CRYP_DMAREQ(CRYP_DMAReq));
  //assert(IS_FUNCTIONAL_STATE(NewState));
...
{
  /* Check the parameters */
  //assert(IS_CRYP_CONFIG_IT(CRYP_IT));
  //assert(IS_FUNCTIONAL_STATE(NewState));
...
FlagStatus CRYP_GetFlagStatus(uint8_t CRYP_FLAG)
{
  ...
  /* Check the parameters */
  //assert(IS_CRYP_GET_FLAG(CRYP_FLAG));
