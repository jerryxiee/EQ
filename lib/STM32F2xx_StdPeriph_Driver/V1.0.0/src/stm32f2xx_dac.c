...
void DAC_Init(uint32_t DAC_Channel, DAC_InitTypeDef* DAC_InitStruct)
{
  ...
  /*
  assert(IS_DAC_TRIGGER(DAC_InitStruct->DAC_Trigger));
  assert(IS_DAC_GENERATE_WAVE(DAC_InitStruct->DAC_WaveGeneration));
  assert(IS_DAC_LFSR_UNMASK_TRIANGLE_AMPLITUDE(DAC_InitStruct->DAC_LFSRUnmask_TriangleAmplitude));
  assert(IS_DAC_OUTPUT_BUFFER_STATE(DAC_InitStruct->DAC_OutputBuffer));
*/
...
void DAC_Cmd(uint32_t DAC_Channel, FunctionalState NewState)
{
  ...
  ///assert(IS_DAC_CHANNEL(DAC_Channel));
...
{  
  ...
  /* Check the parameters */
  //assert(IS_DAC_ALIGN(DAC_Align));
  //assert(IS_DAC_DATA(Data));
...
void DAC_SetDualChannelData(uint32_t DAC_Align, uint16_t Data2, uint16_t Data1)
{
  ...
  /* Check the parameters */
  //assert(IS_DAC_ALIGN(DAC_Align));
  //assert(IS_DAC_DATA(Data1));
  //assert(IS_DAC_DATA(Data2));
