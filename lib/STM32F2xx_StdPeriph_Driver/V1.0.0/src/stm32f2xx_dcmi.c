...
void DCMI_Init(DCMI_InitTypeDef* DCMI_InitStruct)
{
  ...
  /*
  assert(IS_DCMI_CAPTURE_MODE(DCMI_InitStruct->DCMI_CaptureMode));
  assert(IS_DCMI_SYNCHRO(DCMI_InitStruct->DCMI_SynchroMode));
  assert(IS_DCMI_PCKPOLARITY(DCMI_InitStruct->DCMI_PCKPolarity));
  assert(IS_DCMI_VSPOLARITY(DCMI_InitStruct->DCMI_VSPolarity));
  assert(IS_DCMI_HSPOLARITY(DCMI_InitStruct->DCMI_HSPolarity));
  assert(IS_DCMI_CAPTURE_RATE(DCMI_InitStruct->DCMI_CaptureRate));
  assert(IS_DCMI_EXTENDED_DATA(DCMI_InitStruct->DCMI_ExtendedDataMode));
*/
...
{
  /* Check the parameters */
  //assert(IS_DCMI_CONFIG_IT(DCMI_IT));
  ///assert(IS_FUNCTIONAL_STATE(NewState));
...
FlagStatus DCMI_GetFlagStatus(uint16_t DCMI_FLAG)
{
  ...
  /* Check the parameters */
  //assert(IS_DCMI_GET_FLAG(DCMI_FLAG));
...
{
  /* Check the parameters */
  //assert(IS_DCMI_CLEAR_FLAG(DCMI_FLAG));
