...
void DMA_DeInit(DMA_Stream_TypeDef* DMAy_Streamx)
{
  ...
  //assert(IS_DMA_ALL_PERIPH(DMAy_Streamx));
...
void DMA_Init(DMA_Stream_TypeDef* DMAy_Streamx, DMA_InitTypeDef* DMA_InitStruct)
{
  ...
  /*
  assert(IS_DMA_ALL_PERIPH(DMAy_Streamx));
  assert(IS_DMA_CHANNEL(DMA_InitStruct->DMA_Channel));
  assert(IS_DMA_DIRECTION(DMA_InitStruct->DMA_DIR));
  ...
  assert(IS_DMA_FIFO_MODE_STATE(DMA_InitStruct->DMA_FIFOMode));
  assert(IS_DMA_FIFO_THRESHOLD(DMA_InitStruct->DMA_FIFOThreshold));
  assert(IS_DMA_MEMORY_BURST(DMA_InitStruct->DMA_MemoryBurst));
  assert(IS_DMA_PERIPHERAL_BURST(DMA_InitStruct->DMA_PeripheralBurst));
*/
...
void DMA_PeriphIncOffsetSizeConfig(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_Pincos)
{
  /* Check the parameters */
  //assert(IS_DMA_ALL_PERIPH(DMAy_Streamx));
  //assert(IS_DMA_PINCOS_SIZE(DMA_Pincos));
...
void DMA_FlowControllerConfig(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_FlowCtrl)
{
  /* Check the parameters */
  //assert(IS_DMA_ALL_PERIPH(DMAy_Streamx));
  //assert(IS_DMA_FLOW_CTRL(DMA_FlowCtrl));
...
{  
  /* Check the parameters */
  assert(IS_DMA_ALL_PERIPH(DMAy_Streamx));
  assert(IS_DMA_CURRENT_MEM(DMA_CurrentMemory));
                           ...
                           uint32_t DMA_MemoryTarget)
{
  /* Check the parameters */
  assert(IS_DMA_ALL_PERIPH(DMAy_Streamx));
  assert(IS_DMA_CURRENT_MEM(DMA_MemoryTarget));
