...
void HASH_Init(HASH_InitTypeDef* HASH_InitStruct)
{
  ...
  assert(IS_HASH_ALGOSELECTION(HASH_InitStruct->HASH_AlgoSelection));
  assert(IS_HASH_DATATYPE(HASH_InitStruct->HASH_DataType));
  assert(IS_HASH_ALGOMODE(HASH_InitStruct->HASH_AlgoMode));
  ...
  {
    assert(IS_HASH_HMAC_KEYTYPE(HASH_InitStruct->HASH_HMACKeyType));
...
{
  /* Check the parameters */
  assert(IS_HASH_VALIDBITSNUMBER(ValidNumber));
...
FlagStatus HASH_GetFlagStatus(uint16_t HASH_FLAG)
{
  ...
  /* Check the parameters */
  assert(IS_HASH_GET_FLAG(HASH_FLAG));
...
{
  /* Check the parameters */
  assert(IS_HASH_CLEAR_FLAG(HASH_FLAG));
