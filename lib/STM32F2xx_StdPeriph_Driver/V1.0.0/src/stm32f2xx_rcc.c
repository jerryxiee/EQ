...
void RCC_PLLConfig(uint32_t RCC_PLLSource, uint32_t PLLM, uint32_t PLLN, uint32_t PLLP, uint32_t PLLQ)
{
  ...
  assert(IS_RCC_PLLM_VALUE(PLLM));
  assert(IS_RCC_PLLN_VALUE(PLLN));
  assert(IS_RCC_PLLP_VALUE(PLLP));
  assert(IS_RCC_PLLQ_VALUE(PLLQ));
...
{
  /* Check the parameters */
  assert(IS_RCC_PLLI2SN_VALUE(PLLI2SN));
  assert(IS_RCC_PLLI2SR_VALUE(PLLI2SR));
...
void RCC_MCO1Config(uint32_t RCC_MCO1Source, uint32_t RCC_MCO1Div)
{
  ...
  /* Check the parameters */
  assert(IS_RCC_MCO1SOURCE(RCC_MCO1Source));
  assert(IS_RCC_MCO1DIV(RCC_MCO1Div));  
...
void RCC_MCO2Config(uint32_t RCC_MCO2Source, uint32_t RCC_MCO2Div)
{
  ...
  /* Check the parameters */
  assert(IS_RCC_MCO2SOURCE(RCC_MCO2Source));
  assert(IS_RCC_MCO2DIV(RCC_MCO2Div));
...
{
  /* Check the parameters */
  assert(IS_RCC_I2SCLK_SOURCE(RCC_I2SCLKSource));
...
{
  /* Check the parameters */
  assert(IS_RCC_AHB1_CLOCK_PERIPH(RCC_AHB1Periph));
...
{
  /* Check the parameters */
  assert(IS_RCC_AHB2_PERIPH(RCC_AHB2Periph));
  assert(IS_FUNCTIONAL_STATE(NewState));
...
{
  /* Check the parameters */
  assert(IS_RCC_AHB3_PERIPH(RCC_AHB3Periph));  
  assert(IS_FUNCTIONAL_STATE(NewState));
...
{
  /* Check the parameters */
  assert(IS_RCC_AHB1_RESET_PERIPH(RCC_AHB1Periph));
  assert(IS_FUNCTIONAL_STATE(NewState));
...
{
  /* Check the parameters */
  assert(IS_RCC_APB2_RESET_PERIPH(RCC_APB2Periph));
  assert(IS_FUNCTIONAL_STATE(NewState));
  if (NewState != DISABLE)
  {
...
{
  /* Check the parameters */
  assert(IS_RCC_AHB1_LPMODE_PERIPH(RCC_AHB1Periph));
  assert(IS_FUNCTIONAL_STATE(NewState));
  if (NewState != DISABLE)
  {
