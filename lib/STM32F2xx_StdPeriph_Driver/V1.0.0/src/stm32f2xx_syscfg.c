...
void SYSCFG_MemoryRemapConfig(uint8_t SYSCFG_MemoryRemap)
{
  ...
  assert(IS_SYSCFG_MEMORY_REMAP_CONFING(SYSCFG_MemoryRemap));
...
void SYSCFG_EXTILineConfig(uint8_t EXTI_PortSourceGPIOx, uint8_t EXTI_PinSourcex)
{
  ...
  /* Check the parameters */
  assert(IS_EXTI_PORT_SOURCE(EXTI_PortSourceGPIOx));
  assert(IS_EXTI_PIN_SOURCE(EXTI_PinSourcex));
...
{ 
  assert(IS_SYSCFG_ETH_MEDIA_INTERFACE(SYSCFG_ETH_MediaInterface)); 
