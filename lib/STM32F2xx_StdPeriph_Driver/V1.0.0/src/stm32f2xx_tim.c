...
void TIM_CounterModeConfig(TIM_TypeDef* TIMx, uint16_t TIM_CounterMode)
{
  ...
  /* Check the parameters */
  assert(IS_TIM_LIST3_PERIPH(TIMx));
...
{
  /* Check the parameters */
  assert(IS_TIM_LIST1_PERIPH(TIMx));
...
void TIM_OC1Init(TIM_TypeDef* TIMx, TIM_OCInitTypeDef* TIM_OCInitStruct)
{
  ...
  assert(IS_TIM_OUTPUT_STATE(TIM_OCInitStruct->TIM_OutputState));
  ...
  if((TIMx == TIM1) || (TIMx == TIM8))
  {
    assert(IS_TIM_OUTPUTN_STATE(TIM_OCInitStruct->TIM_OutputNState));
    assert(IS_TIM_OCN_POLARITY(TIM_OCInitStruct->TIM_OCNPolarity));
    assert(IS_TIM_OCNIDLE_STATE(TIM_OCInitStruct->TIM_OCNIdleState));
    assert(IS_TIM_OCIDLE_STATE(TIM_OCInitStruct->TIM_OCIdleState));
...
void TIM_OC2Init(TIM_TypeDef* TIMx, TIM_OCInitTypeDef* TIM_OCInitStruct)
{
  ...
  /* Check the parameters */
  assert(IS_TIM_LIST2_PERIPH(TIMx)); 
  assert(IS_TIM_OC_MODE(TIM_OCInitStruct->TIM_OCMode));
  assert(IS_TIM_OUTPUT_STATE(TIM_OCInitStruct->TIM_OutputState));
  assert(IS_TIM_OC_POLARITY(TIM_OCInitStruct->TIM_OCPolarity));   
...
void TIM_SelectOCxM(TIM_TypeDef* TIMx, uint16_t TIM_Channel, uint16_t TIM_OCMode)
{
  ...
  /* Check the parameters */
  assert(IS_TIM_LIST1_PERIPH(TIMx));
  assert(IS_TIM_CHANNEL(TIM_Channel));
  assert(IS_TIM_OCM(TIM_OCMode));
...
void TIM_OC1NPolarityConfig(TIM_TypeDef* TIMx, uint16_t TIM_OCNPolarity)
{
  uint16_t tmpccer = 0;
  /* Check the parameters */
  assert(IS_TIM_LIST4_PERIPH(TIMx));
  assert(IS_TIM_OCN_POLARITY(TIM_OCNPolarity));
...
void TIM_CCxNCmd(TIM_TypeDef* TIMx, uint16_t TIM_Channel, uint16_t TIM_CCxN)
{
  ...
  /* Check the parameters */
  assert(IS_TIM_LIST4_PERIPH(TIMx));
  assert(IS_TIM_COMPLEMENTARY_CHANNEL(TIM_Channel));
  assert(IS_TIM_CCXN(TIM_CCxN));
...
void TIM_SetIC1Prescaler(TIM_TypeDef* TIMx, uint16_t TIM_ICPSC)
{
  /* Check the parameters */
  assert(IS_TIM_LIST1_PERIPH(TIMx));
  assert(IS_TIM_IC_PRESCALER(TIM_ICPSC));
...
void TIM_BDTRConfig(TIM_TypeDef* TIMx, TIM_BDTRInitTypeDef *TIM_BDTRInitStruct)
{
  /* Check the parameters */
  assert(IS_TIM_LIST4_PERIPH(TIMx));
  assert(IS_TIM_OSSR_STATE(TIM_BDTRInitStruct->TIM_OSSRState));
  assert(IS_TIM_OSSI_STATE(TIM_BDTRInitStruct->TIM_OSSIState));
  assert(IS_TIM_LOCK_LEVEL(TIM_BDTRInitStruct->TIM_LOCKLevel));
  assert(IS_TIM_BREAK_STATE(TIM_BDTRInitStruct->TIM_Break));
  assert(IS_TIM_BREAK_POLARITY(TIM_BDTRInitStruct->TIM_BreakPolarity));
  assert(IS_TIM_AUTOMATIC_OUTPUT_STATE(TIM_BDTRInitStruct->TIM_AutomaticOutput));
...
{ 
  /* Check the parameters */
  assert(IS_TIM_LIST5_PERIPH(TIMx)); 
                            ...
                            uint16_t TIM_ExtTRGPolarity, uint16_t ExtTRGFilter)
{
  ...
  assert(IS_TIM_EXT_FILTER(ExtTRGFilter));
...
{
 /* Check the parameters */
  assert(IS_TIM_LIST6_PERIPH(TIMx));
  assert(IS_TIM_REMAP(TIM_Remap));
