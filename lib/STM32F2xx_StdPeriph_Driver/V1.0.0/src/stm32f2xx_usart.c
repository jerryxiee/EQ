...
void USART_Init(USART_TypeDef* USARTx, USART_InitTypeDef* USART_InitStruct)
{
  ...
  {
    assert(IS_USART_1236_PERIPH(USARTx));
  }
...
void USART_ClockInit(USART_TypeDef* USARTx, USART_ClockInitTypeDef* USART_ClockInitStruct)
{
  ...
  /* Check the parameters */
  assert(IS_USART_1236_PERIPH(USARTx));
  assert(IS_USART_CLOCK(USART_ClockInitStruct->USART_Clock));
  assert(IS_USART_CPOL(USART_ClockInitStruct->USART_CPOL));
  assert(IS_USART_CPHA(USART_ClockInitStruct->USART_CPHA));
  assert(IS_USART_LASTBIT(USART_ClockInitStruct->USART_LastBit));
...
void USART_ClearITPendingBit(USART_TypeDef* USARTx, uint16_t USART_IT)
{
  ...
  /* Check the parameters */
  assert(IS_USART_ALL_PERIPH(USARTx));
  assert(IS_USART_CLEAR_IT(USART_IT)); 
